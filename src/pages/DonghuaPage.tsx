...
import { Play, ChevronLeft, ChevronRight, Star, Info, Flame, Clock, Video } from "lucide-react";
import { fetchContent, getPosterUrl, formatRating, Content } from "@/services/api";

const GENRES = ["Action", "Fantasy", "Adventure", "Martial Arts", "Cultivation", "Romance", "Sci-fi", "Demon", "Drama", "Historical", "Supernatural", "Isekai", "Comedy", "School"];

function PosterCard({ item }: { item: Content }) {
  return (
    <Link to={`/donghua/${item.slug}`} className="group block">
...
function Row({ title, icon, params }: { title: string; icon: React.ReactNode; params: any }) {
  ...
  useEffect(() => { fetchContent({ type: 'donghua', limit: 14, ...params }).then(d => setItems(d.content)).catch(() => {}); }, []);
...
const DonghuaPage = () => {
  ...
  useEffect(() => {
    fetchContent({ type: 'donghua', limit: 6, order_by: 'rating', order_dir: 'DESC' })
  ...
  useEffect(() => {
    ...
    fetchContent({ type: 'donghua', status: status === 'all' ? undefined : status, genre: genre || undefined, page, limit: 24, order_by: 'updated_at', order_dir: 'DESC' })
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="relative h-[58vh] min-h-[360px] flex items-end">
        <div className="absolute inset-0">
          {hero && <img referrerPolicy="no-referrer" key={hero.id} src={getPosterUrl(hero.banner_url || hero.poster_url)} alt={hero.title} className="w-full h-full object-cover object-center animate-fade-in" onError={(e) => { (e.target as HTMLImageElement).style.opacity = '0'; }} />}
          <div className="absolute inset-0 bg-gradient-to-t from-background via-background/60 to-background/20" />
          <div className="absolute inset-0 bg-gradient-to-r from-background/80 to-transparent" />
        </div>
        <div className="container mx-auto px-4 relative z-10 pb-8">
          <div className="flex items-center gap-2 mb-2">
            <span className="px-2 py-0.5 rounded bg-primary text-white text-xs font-bold">★ Top Donghua</span>
          ...
          <h1 className="font-display text-3xl sm:text-5xl font-extrabold mb-3 max-w-xl leading-none">{hero?.title || 'Donghua'}</h1>
          <div className="flex gap-3">
            <Link to={hero ? `/donghua/${hero.slug}` : '#'} className="btn-textured inline-flex items-center gap-2 px-6 py-2.5 rounded-lg text-white font-semibold text-sm transition-all shadow-lg"><Play className="w-4 h-4 fill-current" /> Tonton</Link>
            <Link to={hero ? `/donghua/${hero.slug}` : '#'} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-secondary/80 hover:bg-secondary text-foreground font-medium text-sm transition-colors"><Info className="w-4 h-4" /> Detail</Link>
          </div>
      ...
      <main className="container mx-auto px-4 -mt-2 relative z-10 pb-12">
        ...
        <Row title="Ongoing" icon={<Video className="w-5 h-5" />} params={{ status: 'ongoing', order_by: 'updated_at', order_dir: 'DESC' }} />
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            <h2 className="font-display text-lg font-bold">{genre || (status !== 'all' ? status : 'Semua Donghua')}</h2>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3 md:gap-4">
            {loading ? Array(18).fill(0).map((_, i) => <div key={i}><div className="aspect-[2/3] rounded-lg bg-secondary animate-pulse" /><div className="mt-2 h-3 bg-secondary rounded animate-pulse w-4/5" /></div>)
              : items.length === 0 ? <div className="col-span-full text-center py-16 text-muted-foreground"><p className="text-4xl mb-3">📭</p><p>Tidak ada donghua</p></div>
